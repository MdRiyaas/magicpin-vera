...
def validate_result(result, trigger, customer):
    errors = []
    if not result.get("body"): errors.append("empty body")
    if result.get("cta") not in {"open_ended","binary_yes_stop","none"}: errors.append(f"bad cta: {result.get('cta')}")
    if result.get("send_as") not in {"vera","merchant_on_behalf"}: errors.append(f"bad send_as: {result.get('send_as')}")
